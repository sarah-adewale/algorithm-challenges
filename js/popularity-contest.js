...
function calcAverageLikes(data){
    // copy the data
    const newData = data
    // use reduce to get the sum of like
    const total = newData.reduce((acc, c) => acc + c.likes, 0)
    // get the average by divided the sum of like by the length of the data
    const average = total / newData.length
    // return the average
    return average
}
