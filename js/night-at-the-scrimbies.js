...
//    copy data
const newData = data
// use reduce to flatten the hosts
return newData.reduce((acc, c) => acc.concat(c.hosts), [])
...
function assignAwards(data){
 ...
 return hosts.map((item) => {
    const randomAwards = Math.floor(Math.random() * awards.length)
    return `${awards[randomAwards]} ${item}`
 })
}
